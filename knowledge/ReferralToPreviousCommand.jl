...
if you need clarification about the previous command or if the previous context seems to be needed, just read from `tasks[:latest_task]` in your agency `next`
for example
latest_task = tasks[:latest_task]
last_input, last_output = latest_task.input, latest_task.output
next(who, "... something including \$last_input, \$last_output ...", complexity)
"""