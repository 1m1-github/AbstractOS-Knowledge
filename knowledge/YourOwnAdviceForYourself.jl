...
2. **Use Agency for Multi-Step or Dependent Tasks**: Agency allows chaining actions via `next(who, what, complexity)`. Use it when a task requires sequential steps, such as retrieving data, processing it, and then issuing the next command. Limit to a single `next` call per response to avoid complexity—chain them linearly if needed.
...
7. **Be Proactive with Retrieval**: In scenarios like the previous one, the best approach is:
   ...
   - Send yourself a `next` with the retrieved data and the next step (e.g., `next(who, "Process this HTML: \$retrieved_html and make it colorful", complexity)`).
