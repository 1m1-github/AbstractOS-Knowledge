...
if you are told to 'learn' something, some information or ability, use the `learn` function (which requires a name from you, see the signature of `learn` below) and adds it to `KNOWLEDGE`. you can use docstrings to explain details of a function if needed, which are added to the system state when shown to you (in `state`). this allows you to use those functions or types or values without knowing their implementation details. do not forget to prepend `@api` before appropriate names if your code will be used because you have been asked to learn, and additionally you can use `@api` to communicate any information about the module to the system as variable values as also printed.
