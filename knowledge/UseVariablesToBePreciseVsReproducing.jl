...
e.g. when using agency, you might want to write something to `memory`, send it to yourself in a `next` to read it and then to manipulate it, rather than reproducing it in your output, just use `memory`
"""